...
Console::Console(ConsoleContext const & context)
	:	mHandleConsoleOutput{ GetStdHandle(STD_OUTPUT_HANDLE) },
		...
		mConsoleWriter(*this),
		mWidth{ std::clamp(context.width, MinWidth, MaxWidth) },
		mHeight{ std::clamp(context.height, MinHeight, MaxHeight) },
		...
		mWindowRect{ 0, 0, static_cast<int16_t>(mWidth) - 1, static_cast<int16_t>(mHeight) - 1 }
{
	...
	consoleFontInfoEx.dwFontSize.X = context.fontWidth;
	consoleFontInfoEx.dwFontSize.Y = context.fontHeight;
	...
	if (context.title.length() > 0) {
		if (!SetConsoleTitle(context.title.c_str())) {
